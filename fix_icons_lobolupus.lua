...
if g_resources.fileExists(timeSpellFile) then
    ...
    elseif not ok then
        return onError("Erro carregando arquivo TimeSpell.json: " .. result)
    end
end
...
local function saveConfig()
    ...
    if ok then g_resources.writeFileContents(timeSpellFile, result)
    else onError("Erro salvando TimeSpell.json: " .. result) end
end
...
-- ÍCONE
-----------------------------
potdeath = addIcon("PotDeath", { item = 11808, text = "Death" }, potdeath)


-- Macro Auto-Pill
local autopill = macro(200, "Auto-Pill", function()
...
onTalk(function(name, _, _, text)
  if name == player:getName() then
    if text == 'Crock Crock' or text == 'Nhack Nhack' then
      userPill = os.time() + 30
    end
  end
end)

-- Adiciona ícone para Auto-Pill
autopill = addIcon("autopill", {item=11821, text="Pill"}, autopill)
